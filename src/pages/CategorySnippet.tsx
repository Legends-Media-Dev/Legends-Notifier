...
import { Plus, GripVertical, Trash2, Loader2, ShoppingBag, LayoutGrid } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
import {
  fetchCollections,
  fetchCategorySnippetInfo,
  flattenCollectionMap,
  pushCategorySnippetInfo,
  ShopCollection,
  type CategorySnippetDoc,
} from '../lib/api';
...
function docToSelected(doc: CategorySnippetDoc): SelectedCollection[] {
  const ordered = flattenCollectionMap([doc]);
...
const CategorySnippet = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setLoadError(null);
        const [shopifyData, snippetData] = await Promise.all([
          fetchCollections(),
          fetchCategorySnippetInfo().catch(() => ({ items: [], total: 0 })),
        ]);
        if (cancelled) return;
        setAvailableCollections(Array.isArray(shopifyData) ? shopifyData : []);
        const first = snippetData.items?.[0];
        if (first) {
          setDocId(first.id);
          setSectionTitle(first.sectionTitle ?? '');
          setSelected(docToSelected(first));
        }
      } catch (error: unknown) {
        if (!cancelled) {
          ...
          setLoadError(msg);
          setToast({ isVisible: true, message: 'Failed to load category snippet', type: 'error' });
        }
      } finally {
  ...
  const isEvenCount = selected.length > 0 && selected.length % 2 === 0;
  const canSave = isEvenCount && selected.length > 0;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = docId
        ? { docId, sectionTitle: sectionTitle.trim() || undefined, COLLECTION_MAP, ORDER, DISPLAY_NAMES }
        : { sectionTitle: sectionTitle.trim() || undefined, COLLECTION_MAP, ORDER, DISPLAY_NAMES };
      const result = await pushCategorySnippetInfo(payload);
      if (result.id && !docId) setDocId(result.id);
      showToast(result.created ? 'Category snippet saved.' : 'Category snippet updated.');
    } catch (error) {
      showToast('Failed to save category snippet', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen pb-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Category Snippet</h1>
          <p className="mt-2 text-gray-600">
            Set the snippet title and choose an even number of collections to show on the app home. Reorder and set display names.
          </p>
        </div>
        ...
        {loadingCollections ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-8 text-center">
              <p className="text-red-600 font-medium">Could not load</p>
              <p className="text-sm text-gray-500 mt-2">Error: {loadError}</p>
              <button
                type="button"
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-6">
              <div className="px-6 py-4 border-b border-gray-200 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500">Title shown above the category collection list (e.g. Explore)</p>
                </div>
              </div>
              <div className="p-6">
                <input
                  ...
                  className="w-full max-w-md px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-apple-blue focus:border-transparent"
                />
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Add an <strong>even number</strong> of collections. You have {selected.length} selected.
                      {selected.length > 0 && selected.length % 2 !== 0 && (
                        <span className="text-amber-600 ml-1">(Add or remove one to save.)</span>
                      )}
                    </p>
                  </div>
                </div>
              ...
              <div className="divide-y divide-gray-200">
                {selectedSorted.length === 0 ? (
                  <div className="px-6 py-12 text-center text-gray-500">
                    ...
                    <p className="text-sm mt-1">Click “Add collection” and choose an even number (2, 4, 6, …) to save.</p>
                  </div>
                ) : (
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  {!isEvenCount && (
                    <p className="text-sm text-amber-600 font-medium">
                      Include an even number of collections (currently {selected.length}) to save.
                    </p>
                  )}
